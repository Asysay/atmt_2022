...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            self.lex_context_projection = nn.Linear(embed_dim, embed_dim, bias=False)
            self.lex_final_projection = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    lexical_contexts.append(torch.tanh(self.lex_context_projection(lexical_context)) + lexical_context)
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            decoder_output += self.lex_final_projection(lexical_contexts)
            # TODO: --------------------------------------------------------------------- /CUT
